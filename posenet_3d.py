...
def conv_block(input_tensor, kernel_size, filters, stage, block, mystrides=(2, 2)):
    ...
    x = Convolution2D(nb_filter1, (1, 1),
                      name=conv_name_base + '2a', use_bias=False, strides=mystrides)(input_tensor)
    x = BatchNormalization(epsilon=eps, axis=bn_axis, name=bn_name_base + '2a')(x)
    x = Scale(axis=bn_axis, name=scale_name_base + '2a')(x)
    x = Activation('relu', name=conv_name_base + '2a_relu')(x)
    ...
    shortcut = Convolution2D(nb_filter3, (1, 1), 
                             name=conv_name_base + '1', use_bias=False, strides=mystrides)(input_tensor)
...
def resnet101_model(img_rows, img_cols, color_type=1, num_classes=None):
    ...
    x = Convolution2D(64, (7, 7), name='conv1', use_bias=False, strides = (2, 2))(x)
    x = BatchNormalization(epsilon=eps, axis=bn_axis, name='bn_conv1')(x)
    x = Scale(axis=bn_axis, name='scale_conv1')(x)
    x = Activation('relu', name='conv1_relu')(x)
    x = MaxPooling2D((3, 3), strides=(2, 2), name='pool1')(x)

    x = conv_block(x, 3, [64, 64, 256], stage=2, block='a', mystrides=(1, 1))
